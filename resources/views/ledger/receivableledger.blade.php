                ...
                <div class="card">
                    <div class="card-header">
                        <h3> @foreach ($accountnames as $accountname)
                            ...
                            <form  action="{{route('accReceivable_ledger_search')}}" method ="POST">
                        @csrf
                        <br>
                        <div class="container">
                            <div class="row">
                    ...
                    $tbalance = 0;
                    ?>                     
                  @forelse ($assets as $assetamt)

                    @if($assetamt->transaction_type =='Receive')

                        <?php $tbalance = $tbalance - $assetamt->at_amount;?>

                    @elseif( $assetamt->payment_type == 'Account Receivable' || $assetamt->transaction_type == 'Payment')
        ...
        <table class="table table-bordered table-hover"   id="example">
            ...
            @forelse ($assets as $asset)
                <tr>
                    ...
                    <td>{{$asset->ref}}</td>
                    @if($asset->transaction_type =='Receive')
                        <td></td>
                        <td>{{$asset->at_amount}}</td>
                        <td>{{ round($tbalance, 2) }} </td>
                        <?php $tbalance = $tbalance + $asset->at_amount;?>

                    @elseif( $asset->payment_type == 'Account Receivable' || $asset->transaction_type == 'Payment')
                        <td>{{$asset->at_amount}}</td>
                        <td></td>
                        <td>{{ round($tbalance, 2) }} </td>
                        <?php $tbalance = $tbalance - $asset->at_amount;?>
...
@section('scripts')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#receivable_ledger").addClass('active');
        });
    </script>
@endsection
