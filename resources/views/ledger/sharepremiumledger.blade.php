            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3> Share Premium + Capital Reserve Ledger</h3>

                        <form  action="{{route('sharepremium_ledger_search')}}" method ="POST">
                            @csrf
                            <br>
                            <div class="container">
                                <div class="row">
                  ...
                  <!--get total balance -->
                    ...
                    ?> 
                        ...
                        @forelse ($share as $shareamt)

                                @if($shareamt->transaction_type == 'Payment' || $shareamt->transaction_type == 'Add')
                                 
                                 <?php $tbalance =  $tbalance - $shareamt->at_amount ;?>

                                @elseif($shareamt->transaction_type == 'Receive' || $shareamt->transaction_type == 'Reverse')
                        ...
                        <h3>Data: {{$fromDate.' '.$toDate}} </h3>    
                                    ...
                                    @forelse ($share as $share)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{$share->ref}}</td>
                                            @if($share->transaction_type == 'Payment' || $share->transaction_type == 'Add')
                                                <td>{{$share->at_amount}}</td>
                                                <td></td>
                                                <td>{{ round($tbalance, 2) }} </td>
                                                <?php $tbalance =  $tbalance + $share->at_amount ;?>
                                                
                                            @elseif($share->transaction_type == 'Receive' || $share->transaction_type == 'Reverse')
                                                <td></td>
                                                <td>{{$share->at_amount}}</td>
                                                <td>{{ round($tbalance, 2) }} </td>
                                                <?php $tbalance = $tbalance -  $share->at_amount;?>
